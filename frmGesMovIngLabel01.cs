...
namespace APOffice
{
    public partial class frmGesMovIngLabel01 : Form
    {
        ...
        private string _str006_Posz_CodLot = "";
        private string _str006_Font_CodLot = "";
        private string _str006_Posz_DesLot = "";
        private string _str006_Font_DesLot = "";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                if (_strEtiTipo == "02")
                {
                    ...
                    aFon = _str006_Font_CodLot.Split(':');
                    sFonName = aFon[0];
                    fFonDim = (float)Convert.ToDouble(aFon[1]);
                    fSty = FontStyle.Bold;
                    if (aFon[2] == "0")
                    ...
                    e.Graphics.DrawString("Lotto " + s, Font, Brushes.Black, iY, iX);
                    ...
                    //rectF1 = new RectangleF(iY, iX, iYy, iXx);
                    //e.Graphics.DrawString("Preimballato il:", Font, Brushes.Black, rectF1, format1);
                    e.Graphics.DrawString("Da consumarsi entro il:", Font, Brushes.Black, iY, iX);
                    //if (bBordi)
                    //    e.Graphics.DrawRectangle(Pens.Black, Rectangle.Round(rectF1));
                    ...
                    if (_strArtUmi == "NR")
                        s = "SELECT * FROM AnaBarcode WHERE ean_art='" + _strArtCod + "' ORDER BY ean_dti DESC";
                    else
                    ...
                    if (t.Rows.Count > 0)
                    {
                        ...
                        string sEan = (string)t.Rows[0]["ean_ean"];
                        ...
                        e.Graphics.DrawString(sEan, Font, Brushes.Black, iY, iX + 20);
                    }
                ...
                else
                {
                    ...
                    if (File.Exists(sFil))
                    {
                        ...
                        rectF1 = new RectangleF(iY, iX, iYy, iXx);
                        e.Graphics.DrawString(rtxBox.Text, Font, Brushes.Black, rectF1, format1);
                        if (bBordi)
                            e.Graphics.DrawRectangle(Pens.Black, Rectangle.Round(rectF1));
                        //iXProgr += iX;
                    }
                    ...
                    e.Graphics.DrawString("Lotto N." + s, Font, Brushes.Black, iY, iX);
        ...
        {
            string s = "";
            string sFil = Path.GetDirectoryName(_clsDef.FILEINI) + "\\EtiNomeFont1.ini";

            // Controllo sistema Ibrido:
            int etiCodInt = 1;
            int.TryParse(_strEtiCod, out etiCodInt);

            if (etiCodInt >= 900)
            {
                if (string.IsNullOrEmpty(_strConSql)) _strConSql = _clsFun.ConSql("");
                DataTable tLay = _clsFun.FillTabSql("TabEtiFormatiLayout", "SELECT * FROM TabEtiFormatiLayout WHERE eti_cod='" + _strEtiCod + "'", false, _strConSql);
                if (tLay.Rows.Count > 0)
                {
                    DataRow rowLay = tLay.Rows[0];
                    _str001_Print = rowLay["eti_pri"].ToString();
                    _str002_Dim = rowLay["eti_dim"].ToString();

                    Action<string, Action<string[]>> applica = (valore, azione) =>
                    {
                        if (!string.IsNullOrEmpty(valore))
                            azione(valore.Split(';'));
                    };

                    applica(rowLay["eti_s003"].ToString(), a =>
                    {
                        _str003_Posz_NegRag = a.Length > 0 ? a[0] : ""; _str003_Font_NegRag = a.Length > 1 ? a[1] : "";
                        _str003_Posz_NegInd = a.Length > 2 ? a[2] : ""; _str003_Font_NegInd = a.Length > 3 ? a[3] : "";
                        _str003_Posz_RegUls = a.Length > 4 ? a[4] : ""; _str003_Font_RegUls = a.Length > 5 ? a[5] : "";
                    });

                    applica(rowLay["eti_s004"].ToString(), a =>
                    {
                        _str004_Posz_ArtDes = a.Length > 0 ? a[0] : ""; _str004_Font_ArtDes = a.Length > 1 ? a[1] : "";
                    });

                    applica(rowLay["eti_s005"].ToString(), a =>
                    {
                        _str005_Posz_RicDes = a.Length > 0 ? a[0] : ""; _str005_Font_RicDes = a.Length > 1 ? a[1] : "";
                    });

                    applica(rowLay["eti_s006"].ToString(), a =>
                    {
                        _str006_Posz_CodLot = a.Length > 0 ? a[0] : ""; _str006_Font_CodLot = a.Length > 1 ? a[1] : "";
                        _str006_Posz_DesLot = a.Length > 2 ? a[2] : ""; _str006_Font_DesLot = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s007"].ToString(), a =>
                    {
                        _str007_Posz_PriLbl = a.Length > 0 ? a[0] : ""; _str007_Font_PriLbl = a.Length > 1 ? a[1] : "";
                        _str007_Posz_PriDta = a.Length > 2 ? a[2] : ""; _str007_Font_PriDta = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s008"].ToString(), a =>
                    {
                        _str008_Posz_PneLbl = a.Length > 0 ? a[0] : ""; _str008_Font_PneLbl = a.Length > 1 ? a[1] : "";
                        _str008_Posz_PneQta = a.Length > 2 ? a[2] : ""; _str008_Font_PneQta = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s009"].ToString(), a =>
                    {
                        _str009_Posz_ScaLbl = a.Length > 0 ? a[0] : ""; _str009_Font_ScaLbl = a.Length > 1 ? a[1] : "";
                        _str009_Posz_ScaDay = a.Length > 2 ? a[2] : ""; _str009_Font_ScaDay = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s010"].ToString(), a =>
                    {
                        _str010_Posz_PrkLbl = a.Length > 0 ? a[0] : ""; _str010_Font_PrkLbl = a.Length > 1 ? a[1] : "";
                        _str010_Posz_PrkKgr = a.Length > 2 ? a[2] : ""; _str010_Font_PrkKgr = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s011"].ToString(), a =>
                    {
                        _str011_Posz_ImpLbl = a.Length > 0 ? a[0] : ""; _str011_Font_ImpLbl = a.Length > 1 ? a[1] : "";
                        _str011_Posz_ImpVal = a.Length > 2 ? a[2] : ""; _str011_Font_ImpVal = a.Length > 3 ? a[3] : "";
                    });

                    applica(rowLay["eti_s012"].ToString(), a =>
                    {
                        _str012_Posz_LogUno = a.Length > 0 ? a[0] : "";
                        _str012_Posz_LogDue = a.Length > 1 ? a[1] : "";
                    });

                    applica(rowLay["eti_s013"].ToString(), a =>
                    {
                        _str013_Posz_EanCod = a.Length > 0 ? a[0] : "";
                    });
                        ...
                        using (StreamReader sr = new StreamReader(sFil))
                        {
                            ...
                            while ((sRig = sr.ReadLine()) != null)
                            {
                                ...
                                if (sRig.Length > 4)
                                {
                                    ...
                                    {
                                        string[] a = sRig.Substring(4).Split(';');
                                        _str006_Posz_CodLot = a[0];
                                        _str006_Font_CodLot = a[1];
                                        if (a.Length > 3)
                                        {
        ...
        private string SeekLogo(string strTip)
        {
            ...
            string s = _clsFun.FileIniIni("000");
            string sPth = "C:\\ApProject\\Temp\\Img\\Clienti\\";
            if (s != "")
                sPth = sPth.Replace("ApProject", s);
