...
namespace APOffice
{
    public partial class frmSeekTessere2 : Form
    {
        ...
        private void frmSeekFidelity_Load(object sender, EventArgs e)
        {
            ...
            clsUiIcons.RestoreGridColumnWidths(dgv1, "frmSeekTessere2_dgv1");
        }

        private void frmSeekTessere2_FormClosing(object sender, FormClosingEventArgs e)
        {
            clsUiIcons.SaveFormBounds(this);
            clsUiIcons.SaveGridColumnWidths(dgv1, "frmSeekTessere2_dgv1");
        }
        ...
        private void ApplyModernUi()
        {
            ...
            {
                _clsFun.ErrorLog("frmSeekTessere2.ApplyModernUi", ex.Message);
            }
        }
        ...
        private void TabSeek(string strTip)
        {
            ...
            else if (strTip == "COD")
            {
                if (txtCod.Text != "")
                {
                    ...
                    w = "let_tes='" + txtCod.Text + "'";
                }
            }
            if (w != "")
            {
                if (!chkAnn.Checked)
                    w += " AND tes_ann=0 AND let_ann=0 ";
                ...
                if(t.Rows.Count > 0)
                {
                    ...
                    {
                        s = "SELECT * FROM AnaTessLegami WHERE let_ann=0";
                        DataTable tLeg = _clsFun.FillTabSql("", s, false, _strConSql);
                        DataColumn[] Key = new DataColumn[2] 
                        { 
                            tLeg.Columns["let_cod"],
                            tLeg.Columns["let_tes"] 
                        };
                        tLeg.PrimaryKey = Key;
                        ...
                        s = "SELECT ";
                        s += "vet_fid, SUM(vet_imp) AS vet_imp ";
                        s += "FROM GesNegVet ";
                        s += "WHERE (vet_day >= '01/01/2018') AND (vet_fid <> '') ";
                        s += "GROUP BY vet_fid";
                        DataTable tVet = _clsFun.FillTabSql("GesNegVet", s, false, _strConSqlSta);
                        Key = new DataColumn[1] 
                        { 
                            tVet.Columns["vet_fid"],
                        };
                        tVet.PrimaryKey = Key;
                        ...
                        //s += "LEFT(GesNegVep.vep_tip, 2) = 'PU' AND GesNegVep.vep_ean = '')) AS derivedtbl_1 ";
                        ...
                        //s += "SUM(CASE WHEN vep_tip = 'PUM' THEN ABS(vep_imp) * - 1 ELSE vep_imp END) AS vep_pun ";
                        //s += "FROM GesNegVep ";
                        //s += "WHERE GesNegVep.vep_day >= " + _clsFun.DaySql(_dayDti) + " ";
                        //s += "GROUP BY vep_fid";
                        ...
                        s += "FROM GesNegVep ";
                        s += "WHERE ";
                        s += "GesNegVep.vep_day >= " + _clsFun.DaySql(_dayDti) + " AND ";
                        s += "(LEFT(vep_tip, 2) = 'PU' AND vep_ean = '' OR vep_ean = 'PUNTITESSERAX') OR vep_tip='PAG' ";
                        s += "GROUP BY vep_fid";

                        DataTable tSta = _clsFun.FillTabSql("StaVet", s, false, _strConSqlSta);
                        Key = new DataColumn[1] 
                        { 
                            tSta.Columns["vet_fid"] 
                        };
                        tSta.PrimaryKey = Key;

                        foreach (DataRow y in t.Rows)
                        {
                            //if ((string)y["tes_cod"] == "006305")
                            //    Console.WriteLine("ciccio");

                            j1 = tLeg.Select("let_cod='" + y["tes_cod"] + "'");

                            if (j1.Length > 0)
                            {
                                foreach (DataRow yy in j1)
                                {

                                    //j = tSta.Select("vet_fid='" + (string)y["tes_cod"] + "'");
                                    j = tSta.Select("vep_fid='" + (string)yy["let_tes"] + "'");
                                    if (j.Length > 0)
                                    {
                                        //y["TesImp"] = (decimal)j[0]["vep_imp"];
                                        y["TesPun"] = (decimal)j[0]["vep_pun"];
                                    }

                                    j = tVet.Select("vet_fid='" + (string)yy["let_tes"] + "'");
                                    if(j.Length > 0)
        ...
        private void Tes2Pos()
        {
            ...
            string s = "SELECT * FROM AnaTessLegami";
            DataTable tLeg = _clsFun.FillTabSql(TABTESLEG, s, false, _strConSql);
            DataColumn[] Key = new DataColumn[2] 
            { 
                tLeg.Columns["let_cod"],
                tLeg.Columns["let_tes"] 
            ...
            if (!t.Columns.Contains("tes_cli"))
            {
                t.Columns.Add(new DataColumn()
                {
                    DataType = Type.GetType("System.String"),
                    ColumnName = "tes_cli",
                    Caption = "Codice legame",
                    MaxLength = 6,
                    ReadOnly = false,
                    DefaultValue = (String)""
                });
            ...
            foreach (DataRow y in t.Rows)
            {
                ...
                {
                    j = tLeg.Select("let_cod='" + y["tes_cod"] + "'");
                    if (j.Length == 0)
                        sMsg += "Cliente " + (string)y["tes_cod"] + " con LEGAMI TESSERE non definiti " + _clsDef.CRLF;
                    else
                    {
                        for (int i = 0; i < j.Length; i++)
                        {
                            y["tes_gru"] = j[i]["let_gru"];
                            y["tes_cli"] = j[i]["let_cod"];
                            y["tes_cod"] = j[i]["let_tes"];

                            if (((string)j[i]["let_gru"]).Trim() == "")
                                sMsg += "Cliente " + (string)y["tes_cod"] + " con GRUPPO non definito " + _clsDef.CRLF;
                            else if (((string)j[i]["let_tes"]).Trim() == "")
                                sMsg += "Cliente " + (string)y["tes_cod"] + " con TESSERE non definite " + _clsDef.CRLF;
                            else
                                tTmp.ImportRow(y);
                        }
                    }
