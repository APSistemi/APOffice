...
namespace APOffice
{
    public partial class frmGesMovIngLabel05 : Form
    {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                if (_strEtiTipo == "02") 
                {
                    ...
                    //string sLog = _clsFun.PathApOffice("C:\\ApProject\\Temp\\Img\\LogoEti05.png");
                    if (!File.Exists(sLog))
                    {
                        MessageBox.Show("Immagine LOGO etichetta non trovata!", "CONTROLLO LOGO", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        sLog = "";
                    }
                    ...
                    if (_tabLot.Rows.Count > 0)
                    {
                        ...
                        if (sNat != "")
                            sMsg += "NATO IN: " + sNat + _clsDef.CRLF;
                        if (sAll != "")
                            sMsg += "ALLEVATO IN: " + sAll + _clsDef.CRLF;
                        if (sMna != "")
                            sMsg += "MACELLATO IN: " + sMna + " " + sMac + _clsDef.CRLF;
                        if (sSez != "")
                            sMsg += "SEZIONATO IN: " + sSez + " " + sSbo + _clsDef.CRLF;
                    ...
                    //rectF1 = new RectangleF(iY, iX, iYy, iXx);
                    //e.Graphics.DrawString("Preimballato il:", Font, Brushes.Black, rectF1, format1);
                    e.Graphics.DrawString("Da consumarsi entro:", Font, Brushes.Black, iY, iX);
                    //if (bBordi)
                    //    e.Graphics.DrawRectangle(Pens.Black, Rectangle.Round(rectF1));
                    ...
                    * ***/
                    ...
                    string sEan = _strEanCod;

                    s = "SELECT * FROM AnaBarcode WHERE ean_art='" + _strArtCod + "' AND ean_ann=0 AND ean_bil=1";
                    DataTable t = _clsFun.FillTabSql("AnaBarcode", s, true, _strConSql);
                    if (t.Rows.Count > 0)
                    {
                        sEan = (string)t.Rows[0]["ean_ean"];

                        if ((Boolean)t.Rows[0]["ean_ecp"])
                            s = (_decMovQkg * 1000).ToString("00000");
                        else
        ...
        private string FillFont(string strTip)
        {
            ...
            string sFil = Path.GetDirectoryName(_clsDef.FILEINI) + "\\EtiNomeFont5.ini";
