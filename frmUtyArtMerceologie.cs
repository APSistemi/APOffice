...
namespace APOffice
{
    public partial class frmUtyArtMerceologie : Form
    {
        ...
        private void FillTabs()
        {
            ...
            p = "TabEcrLv3";
            s = "SELECT * FROM TabEcrLv3 WHERE tab_ann=0 ORDER BY tab_des";
            t = _clsFun.FillTabSql(p, s, false, _strConSql);
            x = t.NewRow();
            x["tab_cod"] = "";
            x["tab_des"] = "  Non definito";
            t.Rows.InsertAt(x, 0);
            cmbLv3.DataSource = t;
            cmbLv3.DisplayMember = "tab_des";
            cmbLv3.ValueMember = "tab_cod";
            cmbLv3.SelectedValue = "";
            ...
            x["tab_des"] = "  Non definito";
            t.Rows.InsertAt(x, 0);
            cmbNewRep.DataSource = t;
            cmbNewRep.DisplayMember = "tab_des";
            cmbNewRep.ValueMember = "tab_cod";
            cmbNewRep.SelectedValue = "";

            p = "TabEcrLv3";
            s = "SELECT *, tab_lv1 + tab_lv2 + tab_cod AS TabCod FROM TabEcrLv3 WHERE tab_ann=0 ORDER BY tab_des";
            t = _clsFun.FillTabSql(p, s, false, _strConSql);
            x = t.NewRow();
            x["tab_cod"] = "";
            x["tab_des"] = "  Non definito";
            t.Rows.InsertAt(x, 0);
            cmbNewEcr.DataSource = t;
            cmbNewEcr.DisplayMember = "tab_des";
            cmbNewEcr.ValueMember = "TabCod";
            cmbNewEcr.SelectedValue = "";
        }
        ...
        private void FillDati()
        {
            ...
            else if (cmbLv3.SelectedValue != null && cmbLv3.SelectedValue.ToString() != "")
                s += "WHERE art_lv3='" + cmbLv3.SelectedValue.ToString() + "'";
            else if (cmbLv3.SelectedValue != null && cmbLv3.SelectedValue.ToString() != "")
                s += "WHERE art_lv3='" + cmbLv3.SelectedValue.ToString() + "'";
            else if (txtDes.Text.Trim() != "")
                s += "WHERE art_des LIKE '%" + txtDes.Text.Trim() + "%'";
            else if (chkAll.Checked)
                s += "WHERE art_desLIKE '%" + txtDes.Text.Trim() + "%'";
            else
                b = false;
        ...
        private void excelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            sFld += "art_pxc, Pezz, 70, StringLiteral;";
            sFld += "art_pne, Peso netto, 70, StringLiteral;";
            ...
            sFld += "art_cos, Costo, 70, StringLiteral;";
            sFld += "art_prv, Prezzo, 70, StringLiteral;";
        ...
        private void Salva()
        {
            ...
            if (cmbNewEcr.SelectedValue != null && cmbNewEcr.SelectedValue.ToString() != "")
            {
                sEcr = cmbNewEcr.SelectedValue.ToString();

                j = ((DataTable)cmbNewEcr.DataSource).Select("TabCod='" + sEcr + "'");

                sLv1 = (string)j[0]["tab_lv1"];
                sLv2 = (string)j[0]["tab_lv2"];
                sLv3 = (string)j[0]["tab_cod"];
            }

            if (cmbNewRep.SelectedValue != null && cmbNewRep.SelectedValue.ToString() != "")
                sRep = cmbNewRep.SelectedValue.ToString();

            if (sEcr == "" && sRep == "")
                MessageBox.Show("Selezione reparto e merceologia mancante!", "AGGIORNAMENTO MERCEOLOGIE", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
            {
                DataTable t = (DataTable)dgv1.DataSource;
                ...
                foreach (DataRow y in t.Rows)
                {
                    if((Boolean)y["ArtCho"])
                    {
                        ...
                        s = "UPDATE AnaArticoli SET " + sSql + " WHERE art_cod='" + y["art_cod"] + "'";
                        _clsFun.SqlWrite(s, _strConSql);
                    }
                }
